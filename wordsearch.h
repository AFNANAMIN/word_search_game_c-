...
void WordSearch::display() {
	...
	for (int i = 0; i < PUZZLE_SIZE; i++) {
		...
		for (int j = 0; j < PUZZLE_SIZE; j++)
		{
			...
			std::cout << puzzle[i][j] << " ";
			SetConsoleTextAttribute(hStdout, FOREGROUND_BLUE | FOREGROUND_GREEN | FOREGROUND_BLUE);//white on black
		}
		std::cout << "\n";
	}
...
void WordSearch::solve() {
	while (1)
	{
		display();
		string answer;
		cout << "Enter the range (ex. A1:A9 for horizontal) you believe is an answer: ";
		...
		if (check(answer)) {
			PlaySound(TEXT("loop1.wav"), NULL, SND_FILENAME | SND_ASYNC);
			cout << "YOU FOUND A WORD!!!! :) \n\n";
		}
		else {
			cout << "Sorry that's not a word :(\n\n";
		}
		if (words_left() == 0)
		{
			cout << "Woohoo! You Finished the Puzzle!!!!\n\n\n";
			break;
		}
		_sleep(1000);
	}
